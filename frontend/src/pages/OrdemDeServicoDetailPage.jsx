...
import { Box, Typography, Card, CardContent, Divider, Chip, Grid, Container, List, ListItem, ListItemText, IconButton } from '@mui/material';
...
function OrdemDeServicoDetailPage() {
  ...
  const getStatusColor = (status) => {
    switch (status) {
      ...
      case 'EA': return 'warning';
      case 'FN': return 'success';
      case 'PG': return 'success';
      case 'CA': return 'error';
  ...
  const handleDeleteMaterialUtilizado = async (itemId) => {
    if (window.confirm('Tem certeza que deseja remover este material?')) {
      try {
        const token = localStorage.getItem('accessToken');
        const config = { headers: { Authorization: `Bearer ${token}` } };
        await axios.delete(`http://127.0.0.1:8000/api/materiais-utilizados/${itemId}/`, config);
  ...
  const handleDeletePagamento = async (pagamentoId) => {
    if (window.confirm('Tem certeza que deseja remover este pagamento?')) {
      try {
        const token = localStorage.getItem('accessToken');
        const config = { headers: { Authorization: `Bearer ${token}` } };
        await axios.delete(`http://127.0.0.1:8000/api/pagamentos/${pagamentoId}/`, config);
  ...
  return (
    <Container maxWidth="lg" sx={{ my: 4 }}>
      <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', mb: 2 }}>
        <Typography variant="h4" component="h1">OS #{ordem.id}</Typography>
      ...
      <Card sx={{ mb: 3 }}>
        <CardContent>
          ...
          <Typography><strong>Cliente ID:</strong> {ordem.cliente}</Typography>
          <Typography><strong>Data de Abertura:</strong> {new Date(ordem.data_abertura).toLocaleString()}</Typography>
          <Typography variant="h5" sx={{ mt: 2 }}>
            <strong>Valor Total: R$ {ordem.valor_total}</strong>
          </Typography>
        </CardContent>
      </Card>
      ...
      <Grid container spacing={3}>
        <Grid item xs={12} md={6}>
          <Card>
            <CardContent>
              <Typography variant="h6" gutterBottom>Serviços Incluídos</Typography>
              {ordem.servicos_details && ordem.servicos_details.length > 0 ? (
                <List dense>{ordem.servicos_details.map(s => <ListItem key={s.id}><ListItemText primary={s.nome} secondary={`R$ ${s.preco}`} /></ListItem>)}</List>
        ...
        <Grid item xs={12} md={6}>
          <Card>
            <CardContent>
              <Typography variant="h6" gutterBottom>Materiais Utilizados</Typography>
              {ordem.materiais_utilizados && ordem.materiais_utilizados.length > 0 ? (
                <List dense>{ordem.materiais_utilizados.map(m => 
                  <ListItem key={m.id} secondaryAction={
                      <IconButton edge="end" aria-label="delete" onClick={() => handleDeleteMaterialUtilizado(m.id)}>
                        <DeleteIcon />
                      </IconButton>
                    }>
                    <ListItemText primary={`${m.quantidade}x ${m.material.nome}`} />
                  </ListItem>
                )}</List>
        ...
        <Grid item xs={12}>
          <Card>
            <CardContent>
              <Typography variant="h6" gutterBottom>Pagamentos Registrados</Typography>
              {ordem.pagamentos && ordem.pagamentos.length > 0 ? (
                <List dense>{ordem.pagamentos.map(p => 
                  <ListItem key={p.id} secondaryAction={
                      <IconButton edge="end" aria-label="delete" onClick={() => handleDeletePagamento(p.id)}>
                        <DeleteIcon />
                      </IconButton>
                    }>
                    <ListItemText primary={`R$ ${p.valor_pago} (${p.forma_pagamento})`} secondary={`em ${new Date(p.data_pagamento).toLocaleDateString()}`} />
                  </ListItem>
                )}</List>
              ) : <Typography variant="body2">Nenhum pagamento registrado.</Typography>}
            </CardContent>
          </Card>
        </Grid>
      </Grid>
      ...
      <Grid container spacing={3}>
        <Grid item xs={12} md={6}>
          <Typography variant="h6" gutterBottom>Adicionar Material</Typography>
          <AddMaterialForm ordemId={id} onSuccess={fetchOrdemDetalhes} />
        </Grid>
        <Grid item xs={12} md={6}>
          <Typography variant="h6" gutterBottom>Registrar Pagamento</Typography>
