...
import { Box, Typography, Button, Card, CardContent, CardActionArea, Chip, CardActions } from '@mui/material';
...
function OrdensDeServicoPage() {
  ...
  const handleDeleteOrdem = async (ordemId) => {
    if (window.confirm('Tem certeza que deseja deletar esta Ordem de Serviço? Esta ação não pode ser desfeita.')) {
      try {
        const token = localStorage.getItem('accessToken');
        const config = { headers: { Authorization: `Bearer ${token}` } };

        await axios.delete(`http://127.0.0.1:8000/api/ordens/${ordemId}/`, config);
          ...
          ordens.map((ordem) => (
            <Card key={ordem.id} sx={{ mb: 2 }}>
              ...
              <CardActionArea component={Link} to={`/ordens/${ordem.id}`}>
                <CardContent>
                  <Box sx={{ display: 'flex', justifyContent: 'space-between' }}>
                    ...
                    <Chip label={ordem.status} color={getStatusColor(ordem.status)} size="small" />
                  </Box>
                  <Typography color="text.secondary">Cliente ID: {ordem.cliente}</Typography>
              ...
              <CardActions sx={{ justifyContent: 'flex-end' }}>
                <Button size="small" color="error" onClick={() => handleDeleteOrdem(ordem.id)}>
                  Deletar
                </Button>
              </CardActions>
            </Card>
